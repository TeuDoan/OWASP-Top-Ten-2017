...
1. Lỗ hổng IDOR với tham chiếu trực tiếp đến các đối tượng trong cơ sở dữ liệu
...
1. Lỗ hổng IDOR khi tham chiếu trực tiếp tới tệp tin
...
1. Một vài tình huống phổ biến liên quan tới lỗ hổng này và cách để kiểm thử chúng.

1. Giá trị của tham số được sử dụng trực tiếp để lấy thông tin từ Database
...
1. Giá trị của tham số được sử dụng trực tiếp để thực hiện một hoạt động trong hệ thống
...
1. Giá trị của tham số được sử dụng trực tiếp để truy cập vào tài nguyên của hệ thống tập tin
...
1. Giá trị của tham số được sử dụng trực tiếp để  truy cập vào chức năng của ứng dụng
...
1. ## Tìm IDOR ở những nơi không ngờ tới
   1. ### Kiểm tra ID đã mã hóa hoặc bị băm
...
1. ### Nếu không đoán được thì tự chế ra xem
...
1. ### Nhét ID vào mồm bắt phải nhận
...
1. ### Blind IDORs
Sometimes endpoints susceptible to IDOR don’t respond with the leaked information directly. They might lead the application to leak information elsewhere instead: in export files, emails and maybe even text alerts. Đôi khi, các endpoint tương tác với IDOR không phản hồi trực tiếp với thông tin bị rò rỉ. Thay vào đó, chúng có thể khiến ứng dụng rò rỉ thông tin ở nơi khác: trong file xuất ra, email và thậm chí có thể là cảnh báo bằng văn bản.
1. ### Thay đổi request method
Nếu một request method không có tác dụng, có thể thay đổi các method khác như GET, POST, PUT, DELETE, PATCH…

Một mánh phổ biến là thay thế POST bằng PUT,… : the same access controls might not have been implemented!
1. ### Thay đổi định dạng file
...
1. ## Cách gia tăng ảnh hưởng của lỗ hổng IDOR
